...
const { User, City, Country, CompanyKeyFact,CompanyAlbum,CompanyAlbumPhoto } = require("../models/CompanyAssociations");
const {
  Job,
  ContractType,
  StatusJob,
...
const { JobSeeker } = require("../models/JobseekerAssociations");
    ...
    const enrichedCompanies = await Promise.all(
      companies.map(async (company) => {
        ...
        for (const job of jobs) {
          switch (job.statusId) {
            case 2: jobStatusCounts.open++; break;
            case 3: jobStatusCounts.closed++; break;
            case 4: jobStatusCounts.expired++; break;
            // statusId 5 (draft) intentionally ignored
          }
        }
...
exports.createCompanyKeyFact = async (req, res) => {
  try {
    ...
    const existing = await CompanyKeyFact.findOne({
      where: {
        companyId,
        label: sequelize.where(sequelize.fn('LOWER', sequelize.col('label')), label.toLowerCase())
      }
    });
    ...
    for (const job of jobs) {
      switch (job.statusId) {
        ...
        case 4: // Expired
          ...
          break;
        // Drafts (statusId = 5) not included for public profile
      }
    }
