...
exports.createJob = async (req, res) => {
  try {
    const statusId = parseInt(req.query.statusId); // ✅ statusId now comes from StatusJobs table
    console.log("Status of job:",statusId)
    ...
    const {
      ...
      minimumSeniorityId,              // ✅ updated field
      accommodation,
      otherBenefits,
      applicationMethod,
      externalUrl,
    ...
    if (statusId !== 2 && statusId !== 5) {
      validationErrors.push("statusId must be either 2 (open) or 5 (draft)");
    }
    ...
    // ✅ Check if premium is active (valid) or expired
  ...
  console.log(!hasValidPremium&&statusId !== 5);

    if (!hasValidPremium&&statusId !== 5) {
      if (!company.verified) {
    ...
    if (applicationMethod === "external url" && externalUrl) {
      await JobExternalLink.create({
        jobId: job.jobId,
        url: externalUrl, // make sure your JobExternalLink model uses "url"
      });
    }
...
exports.deleteJob = async (req, res) => {
  try {
    ...
    // ✅ Soft delete: Set statusId to "deleted"
    ...
    job.statusId = deletedStatus.statusId;
    await job.save();
...
exports.getAllJobs = async (req, res) => {
  try {
    const jobs = await Job.findAll({
      where: {
        statusId: [1, 2], // ✅ only Open and Closed
      },
      include: [
...
exports.getJobById = async (req, res) => {
  try {
    ...
    // If job is a draft (statusId = 5), only owner company can see
    if (job.statusId === 5) {
    ...
    const nbOfHiredApplicants = await JobApplication.count({
      where: {
        jobId: jobId,
        status: "Hired" // adjust if you store statuses differently
      }
    });

    // ✅ If token is a jobSeeker, check if he applied
    if (jobSeekerId) {
      let applied = false;
    ...
    // If no jobSeeker token, just return the job + applicants info
    res.status(200).json({
      ...job.toJSON(),
      nbOfApplicants,
      nbOfHiredApplicants
...
if (isNaN(jobId)) {
  return res.status(400).json({ message: "Invalid job ID" });
    ...
    // 🔒 Ensure job is not deleted
    const DELETED_STATUS_ID = 7; // replace with your actual Deleted statusId
    if (job.statusId === DELETED_STATUS_ID) {
      return res.status(400).json({ message: "Cannot close a deleted job" });
    }

    // 🔐 Set statusId to the ID for "Closed"
    const CLOSED_STATUS_ID = 3; // replace with your actual Closed statusId
...
exports.makeJobOpen = async (req, res) => {
  try {
    ...
    if (!hasValidPremium) {
      ...
      const currentJobCount = await Job.count({ where: { companyId ,statusId: [2,3,4],} });
      if (currentJobCount >= company.jobLimit) {
        return res.status(403).json({ message: `You reached your job posting limit of ${company.jobLimit}.` });
      }
    }
    ...
    // 🔒 Only Draft (5) or Closed (3) can be changed to Open (2)
    if (![3, 5].includes(job.statusId)) {
...
exports.rejectJobByAdmin = async (req, res) => {
  try {
    ...
    const job = await Job.findByPk(jobId);
    if (!job) return res.status(404).json({ message: "Job not found." });
    if (job.statusId ==7||job.statusId==6) return res.status(400).json({ message: "deleted or rejected jobs can't be rejected." });
